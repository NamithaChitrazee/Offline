...
//-----------------------------------------------------------------------------
    void Data_t::InitEvent(const art::Event* Evt, int DebugLevel) {
      ...
      fListOfSeeds.clear(); // wipe out old seeds
      //kNStations is set as 18
...
//-----------------------------------------------------------------------------
// Initialize and clear diagInfo parameters for PhiZSeed
//-----------------------------------------------------------------------------
      for (int n=0; n<3; n++) {
        h_lineFitter_chi2Dof[n].clear();// chi2Dof of linear fit for each small segment candidates before merging [/per small segment candidate]
      }
//-----------------------------------------------------------------------------
// Initialize and clear diagInfo parameters for HelixFind
//-----------------------------------------------------------------------------
      h_circleFitter_chi2Dof[0].clear();// chi2Dof of circle fit with all combohits in the segment (w/ fixed weight) [/per segment candidate]
      h_circleFitter_chi2Dof[1].clear();// chi2Dof of circle fit with all combohits in the segment (w/ correct weight) [/per segment candidate]
      h_circleFitter_chi2Dof[2].clear();// chi2Dof of circle fit with clean-up combohits (w/ correct weight) [/per segment candidate]
      h_circleFitter_nhits[0].clear();// nhit used in the circle fit (w/ fixed weight) [/per segment candidate]
      h_circleFitter_nhits[1].clear();// nhit used in the circle fit (w/ correct weight) [/per segment candidate]
      h_circleFitter_nhits[2].clear();// nhit used in the circle fit after clean-up (w/ correct weight) [/per segment candidate]
    }
//-----------------------------------------------------------------------------
// called just once
//-----------------------------------------------------------------------------
