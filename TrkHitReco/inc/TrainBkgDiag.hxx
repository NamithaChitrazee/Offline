//Code generated automatically by TMVA for Inference of Model file [TrainBkgDiag.h5] at [Thu Jun 11 22:23:33 2026] 
...
struct Session {
std::vector<float> fTensor_dense17bias0 = std::vector<float>(1);
float * tensor_dense17bias0 = fTensor_dense17bias0.data();
std::vector<float> fTensor_dense15bias0 = std::vector<float>(8);
float * tensor_dense15bias0 = fTensor_dense15bias0.data();
std::vector<float> fTensor_dense17kernel0 = std::vector<float>(4);
float * tensor_dense17kernel0 = fTensor_dense17kernel0.data();
std::vector<float> fTensor_dense16kernel0 = std::vector<float>(32);
float * tensor_dense16kernel0 = fTensor_dense16kernel0.data();
std::vector<float> fTensor_dense16bias0 = std::vector<float>(4);
float * tensor_dense16bias0 = fTensor_dense16bias0.data();
std::vector<float> fTensor_dense15kernel0 = std::vector<float>(16);
float * tensor_dense15kernel0 = fTensor_dense15kernel0.data();
std::vector<float> fTensor_dense17Sigmoid0 = std::vector<float>(1);
float * tensor_dense17Sigmoid0 = fTensor_dense17Sigmoid0.data();
std::vector<float> fTensor_dense17Dense = std::vector<float>(1);
float * tensor_dense17Dense = fTensor_dense17Dense.data();
std::vector<float> fTensor_dense17bias0bcast = std::vector<float>(1);
float * tensor_dense17bias0bcast = fTensor_dense17bias0bcast.data();
std::vector<float> fTensor_dense16Relu0 = std::vector<float>(4);
float * tensor_dense16Relu0 = fTensor_dense16Relu0.data();
std::vector<float> fTensor_dense16bias0bcast = std::vector<float>(4);
float * tensor_dense16bias0bcast = fTensor_dense16bias0bcast.data();
std::vector<float> fTensor_dense16Dense = std::vector<float>(4);
float * tensor_dense16Dense = fTensor_dense16Dense.data();
std::vector<float> fTensor_dense15Relu0 = std::vector<float>(8);
float * tensor_dense15Relu0 = fTensor_dense15Relu0.data();
std::vector<float> fTensor_dense15Dense = std::vector<float>(8);
float * tensor_dense15Dense = fTensor_dense15Dense.data();
std::vector<float> fTensor_dense15bias0bcast = std::vector<float>(8);
...
Session(std::string filename ="") {
   ...
   f.close();
   {
      float * data = TMVA::Experimental::SOFIE::UTILITY::UnidirectionalBroadcast<float>(tensor_dense15bias0,{ 8 }, { 1 , 8 });
      std::copy(data, data + 8, tensor_dense15bias0bcast);
      delete [] data;
   }
   {
      float * data = TMVA::Experimental::SOFIE::UTILITY::UnidirectionalBroadcast<float>(tensor_dense16bias0,{ 4 }, { 1 , 4 });
      std::copy(data, data + 4, tensor_dense16bias0bcast);
      delete [] data;
   }
   {
      float * data = TMVA::Experimental::SOFIE::UTILITY::UnidirectionalBroadcast<float>(tensor_dense17bias0,{ 1 }, { 1 , 1 });
      std::copy(data, data + 1, tensor_dense17bias0bcast);
      delete [] data;
   }
}
...
//--------- Gemm
   ...
   int op_0_lda = 2;
   int op_0_ldb = 8;
   std::copy(tensor_dense15bias0bcast, tensor_dense15bias0bcast + 8, tensor_dense15Dense);
   BLAS::sgemm_(&op_0_transB, &op_0_transA, &op_0_n, &op_0_m, &op_0_k, &op_0_alpha, tensor_dense15kernel0, &op_0_ldb, tensor_input6, &op_0_lda, &op_0_beta, tensor_dense15Dense, &op_0_n);

//------ RELU
   for (int id = 0; id < 8 ; id++){
      tensor_dense15Relu0[id] = ((tensor_dense15Dense[id] > 0 )? tensor_dense15Dense[id] : 0);
   }
...
//--------- Gemm
   ...
   int op_2_lda = 8;
   int op_2_ldb = 4;
   std::copy(tensor_dense16bias0bcast, tensor_dense16bias0bcast + 4, tensor_dense16Dense);
   BLAS::sgemm_(&op_2_transB, &op_2_transA, &op_2_n, &op_2_m, &op_2_k, &op_2_alpha, tensor_dense16kernel0, &op_2_ldb, tensor_dense15Relu0, &op_2_lda, &op_2_beta, tensor_dense16Dense, &op_2_n);
...
//--------- Gemm
   ...
   int op_4_lda = 4;
   int op_4_ldb = 1;
   std::copy(tensor_dense17bias0bcast, tensor_dense17bias0bcast + 1, tensor_dense17Dense);
   BLAS::sgemm_(&op_4_transB, &op_4_transA, &op_4_n, &op_4_m, &op_4_k, &op_4_alpha, tensor_dense17kernel0, &op_4_ldb, tensor_dense16Relu0, &op_4_lda, &op_4_beta, tensor_dense17Dense, &op_4_n);
	for (int id = 0; id < 1 ; id++){
		tensor_dense17Sigmoid0[id] = 1 / (1 + std::exp( - tensor_dense17Dense[id]));
	}
   std::vector<float> ret (tensor_dense17Sigmoid0, tensor_dense17Sigmoid0 + 1);
   return ret;
}
};
